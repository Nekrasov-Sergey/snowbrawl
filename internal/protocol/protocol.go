...
// запоздавшему ответу. Ms заполняет только сервер: это измеренная им задержка, та же самая,
// что видна в лобби и в админке, — клиент её просто показывает и своих замеров не делает.
type Ping struct {
	Seq uint32 `json:"seq,omitempty"`
	Ms  int    `json:"ms,omitempty"` // задержка по прошлому зонду, мс; 0 — ещё не измерена
}
