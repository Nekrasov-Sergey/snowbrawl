...
```js
SnowBrawlSim = {
  SIM_VERSION: '1.12.0',                // semver правил игры, показывается в админке и логах
...
Область попадания (с 1.11.0): модель бойца на клиенте (`web/client/rig.js`) рисуется в полный рост
— голова заметно выше игровой точки `(x,y)`, ноги заметно ниже, — поэтому проверка в
`updateSnowballs` (`hitTest`) уже не просто круг радиуса `radius`: центр области смещён вниз
(`HIT_BOX_DOWN·radius`), сама область — прямоугольник `±HIT_BOX_HALF_W·radius` по X и
`±HIT_BOX_HALF_H·radius` по Y (плюс радиус снежка). Раньше снежок в ноги пролетал мимо кружка
по центру модели. На `canHitTarget`/`aimPath` (порог 20 px по прицельной точке, к `radius` не
привязан) это не влияет.
