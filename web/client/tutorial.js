...
window.SBTutorial = (function () {
  ...
  var SHIELD = [
    ...
    {
      text: 'Пассив «Закалка»: щитовой пузырь гасит одно попадание целиком. Свечение вокруг бойца — пузырь готов. Встаньте на метку и дайте Снайперу попасть.',
      enemy: { role: 'Снайпер', x: SHOOTER.x, y: SHOOTER.y, awake: true, level: 2 },
      marks: function (c) { return c.enemy ? [WALK_MARK, markAt(c.enemy)] : [WALK_MARK]; },
      ...
      hint: function (c) {
        if (c.flags.popAt) return ' Пузырь лопнул — смотрите на свечение: оно проявляется по мере восстановления.';
