...
Версия протокола: **4** (`internal/protocol.Version`, `SBNet.PROTO` в клиенте).
...
сервер) и ответ `pong` (сервер → клиент), а это уже удаление, а не добавление.
...
| `ping` | `{seq, ms?}` | зонд задержки раз в 2 с; клиент обязан ответить `pong` с тем же номером. `ms` — измеренная сервером задержка, её клиент и показывает |
...
Механизм один. Сервер раз в 2 с шлёт `ping{seq, ms}`, клиент обязан ответить `pong{seq}`; по
ответу считается RTT, сглаженный пополам, и публикуется с точностью до миллисекунды. Одно и то же
число идёт в три места: в лобби (поле `ping` у участника плюс отдельный диффингованный
`room.ping`), в админку и обратно клиенту полем `ms` следующего зонда — им живёт индикатор рядом
с точкой соединения. Своего зонда у клиента нет: два независимых замера давали в углу экрана и в
лобби разные числа, а верить измерению клиента нельзя — его видят все участники лобби и админ.
