...
type RoomPlayer struct {
	...
	Ping      int    `json:"ping,omitempty"`    // задержка до сервера, мс; 0 — неизвестна
}

// Ping — тело зонда задержки (SPing) и ответа на него (CPong): одна и та же структура в обе
// стороны, направление задаёт тип сообщения. Номер нужен, чтобы не считать RTT по чужому,
// запоздавшему ответу. Измеренное число зонд не несёт: оно едет отдельным SSelfPing, иначе
// в углу экрана стояло бы значение на цикл зонда старше, чем в лобби.
type Ping struct {
	Seq uint32 `json:"seq,omitempty"`
}

// SelfPing — задержка самого адресата. Отдельное сообщение, потому что угол экрана виден
// везде: в меню, в лобби и в бою, — а room.ping приходит только сидящим в лобби. Уходит на
// том же тике хаба, что и room.ping, поэтому своя строка в составе и индикатор связи меняются
// одним кадром и разойтись не могут.
type SelfPing struct {
	Ms int `json:"ms"` // задержка до сервера, мс; 0 — неизвестна
}
...
// Droppable сообщает, можно ли отбросить это сообщение, если клиент упёрся в лимит частоты.
// Отбрасывать разрешено только то, что клиент шлёт потоком и что не двигает состояние бойца:
// цель движения и прицел. Оба идемпотентны — следующее такое же сообщение через десятки
// миллисекунд отменяет потерю.
//
// Всё остальное отбрасывать нельзя. Потерянный throw оставил бы бойца в замахе навсегда:
// в симуляции chargeStart уже выставил charging, автосброса у человека нет, а новый замах
// отклоняется, пока charging держится. Игрок просто переставал бы стрелять.
func Droppable(env Envelope) bool {
	if env.Type != CInput {
		return false
	}
	var in Input
	...
	return in.Kind == "move" || in.Kind == "aim"
}
