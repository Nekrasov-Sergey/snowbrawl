...
type Player struct {
	...
	// Задержка до сервера. RTT — сглаженная сырая оценка в наносекундах, PingMs — она же в
	// целых миллисекундах: это единственное число, которое видят лобби, админка и сам игрок
	// в углу экрана. Заполняется в internal/hub/ping.go.
	RTT        time.Duration
	RTTAt      time.Time // когда пришёл последний pong
	PingMs     int       // 0 — задержка неизвестна
	PingSeq    uint32
	PingSentAt time.Time // зонд в полёте; нулевое — зонда нет
	// PingSent — какое значение уже уехало игроку в self.ping. Отрицательное означает «ещё
	// не отправляли»: без такого сентинела ноль («задержка неизвестна») не отличить от
	// «не слали ни разу», и после потери зондов угол экрана залипал бы на старом числе.
	PingSent int
