...
не несовместимое изменение. В 0.9.0 версию не поднимали: всё новое (`ping`/`pong` с номером,
`room.ping`, поле `ping` у участника лобби) — это необязательные поля и новые типы, старый клиент
неизвестные типы молча игнорирует. Поднимать пришлось бы, если бы номер `seq` стал обязательным.
...
| `ping` | `{seq?}` | по желанию, сервер ответит `pong` с тем же номером |
| `pong` | `{seq}` | ответ на серверный зонд `ping` (см. ниже): по нему сервер считает задержку |
...
| `pong` | `{seq?}` | ответ на клиентский `ping` с тем же номером |
| `ping` | `{seq}` | зонд задержки от сервера раз в 4 с; клиент обязан ответить `pong` с тем же номером |
...
- **Клиентский**: клиент раз в 2 с шлёт `ping` и по `pong` считает RTT для индикатора рядом с
  точкой соединения. Это его собственная оценка, серверу она не сообщается.
- **Серверный**: сервер раз в 4 с шлёт `ping{seq}`, клиент обязан ответить `pong{seq}`. Так
  задержка попадает в лобби (поле `ping` у участника, плюс отдельный диффингованный `room.ping`)
  и в админку. Значение округляется до 10 мс и публикуется только при заметном сдвиге: сводка
  админки обязана не меняться сама по себе между вызовами, на этом стоит диффинг SSE-потока.
  Ноль или отсутствие поля значит «ещё не измерена» — у ботов, у только что зашедших и у тех,
  кто потерял связь.
