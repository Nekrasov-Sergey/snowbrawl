...
window.SBTutorial = (function () {
  ...
  // Тексты шагов переводятся так же, как везде: ключ — русская строка (см. client/i18n.js).
  var t = window.SBI18n.t;
  ...
  var BASICS = [
    {
      text: t('Дойдите до метки. WASD на клавиатуре, левый стик на телефоне.'),
      marks: function () { return [WALK_MARK]; },
      check: function (c) { return !!c.me && c.dist(c.me, WALK_MARK) < WALK_MARK.r; },
      hint: function (c) { return c.inStep > 10000 ? ' ' + t('Метка — зелёный круг слева от центра арены.') : ''; }
    },
    {
      text: t('Зажмите ЛКМ (на телефоне — правый стик) и держите до полного замаха, потом отпустите. Чем длиннее замах, тем дальше летит снежок.'),
      check: function (c) { return c.flags.fullThrow || (c.flags.chargedFull && c.flags.throws > 0); },
      hint: function (c) {
        return c.flags.throws > 0 && !c.flags.fullThrow ? ' ' + t('Замах был короткий: держите дольше — кружок прицела уходит дальше.') : '';
      }
    },
    {
      text: t('Бросьте ещё два раза. Под полоской здоровья три оранжевых отделения — это запас снежков: каждый выстрел тратит одно, и они заполняются сами.'),
      check: function (c) { return c.flags.throws >= 2; },
      hint: function (c) { return c.inStep > 9000 ? ' ' + t('Нажать можно и с пустым запасом: бросок встанет в очередь и уйдёт, как только отделение заполнится.') : ''; }
    },
    {
      text: t('Попадите снежком в деревянный ящик. Укрытия ловят снежки — обычным броском ящик не пробить.'),
      ...
      hint: function (c) { return c.inStep > 12000 ? ' ' + t('Подойдите ближе и бросьте вполсилы: на слабом замахе снежок идёт низко.') : ''; }
    },
    {
      text: t('Соперник бросает в вас. Спрячьтесь: встаньте так, чтобы ящик оказался между вами.'),
      ...
      hint: function (c) {
        if (!c.flags.exposed) return ' ' + t('Сначала выйдите на открытое место — пусть соперник вас увидит.');
        return c.inStep > 15000 ? ' ' + t('Держите ящик ровно между собой и соперником.') : '';
      }
    },
    {
      text: t('Бой. Три попадания выводят бойца из строя — добейте соперника.'),
      ...
      hint: function (c) { return c.inStep > 25000 ? ' ' + t('Бейте на полном замахе с дистанции: Танк медленный, но вблизи опасен.') : ''; }
    }
  ];
  ...
  function laneHint(c) {
    return c.me && c.dist(c.me, WALK_MARK) > 90
      ? ' ' + t('Встаньте на метку: от спавна бросок упирается в колонну.') : '';
  }

  function fightStep(enemyRole) {
    return {
      text: t('Настоящий бой: примените способность и добейте соперника. Три попадания — и он выведен из строя, а каждый промах стоит снежка из запаса.'),
      ...
      hint: function (c) {
        if (!c.flags.special && c.inStep > 12000) return ' ' + t('Способность ещё не применена — Q, ПКМ или кнопка справа.');
        return c.flags.special && c.inStep > 30000 ? ' ' + t('Бросайте с дистанции и не стойте на месте.') : '';
      }
    };
  }
  ...
  var RUNNER = [
    {
      text: t('Нажмите Q (на телефоне — кнопка справа): рывок бросает бойца в сторону прицела.'),
      marks: function () { return [DASH_MARK]; },
      check: function (c) { return !!c.me && c.me.dash; },
      hint: function (c) { return c.inStep > 12000 ? ' ' + t('На ПК работает и ПКМ. В обучении кулдаун всего 2 секунды.') : ''; }
    },
    {
      text: t('Во время рывка вы неуязвимы. Встаньте на метку, дождитесь броска Снайпера и уйдите рывком из-под снежка.'),
      ...
      hint: function (c) {
        return c.me && c.dist(c.me, WALK_MARK) > 60 && c.inStep > 6000
          ? ' ' + t('Встаньте на метку: только с открытой полосы Снайпер вас видит.') : '';
      }
    },
    {
      text: t('Пассив «Второе дыхание»: на последнем HP Раннер бежит быстрее. Дайте Снайперу попасть дважды и пробегите на одном HP — разницу видно сразу.'),
      enemy: { role: 'Снайпер', x: SHOOTER.x, y: SHOOTER.y, awake: true, level: 2 },
      marks: function (c) { return c.enemy ? [WALK_MARK, markAt(c.enemy)] : [WALK_MARK]; },
      ...
      hint: function (c) {
        if (!c.me) return '';
        if (c.me.hp > 1) return ' ' + t('Не прячьтесь: сначала нужно получить два попадания.');
        return ' ' + t('Теперь просто бегите — отсчёт идёт, пока вы двигаетесь.');
      }
    },
    fightStep('Танк')
  ];
  ...
  var SNIPER = [
    {
      text: t('Нажмите Q: следующий бросок пойдёт по прямой, быстро и далеко — луч прицела станет длиннее.'),
      check: function (c) { return !!c.me && c.me.armed === 'snipe'; }
    },
    {
      text: t('Настильный выстрел идёт у самой земли и вязнет в любой преграде. Встаньте на метку — оттуда линия чистая — и попадите заряженным выстрелом.'),
      ...
      hint: function (c) {
        return c.me && c.dist(c.me, FLAT_SPOT) > 60 ? ' ' + t('У спавна колонна: с места старта выстрел упирается в неё.') : '';
      }
    },
    {
      text: t('Пассив «Точность»: обычный снежок Снайпера летит дальше, чем у любого другого героя. Встаньте на метку и достаньте соперника обычным броском на полном замахе.'),
      ...
      hint: function (c) {
        if (c.me && c.dist(c.me, SNIPE_SPOT) > 60) return ' ' + t('Метка у левого края: с неё до соперника ровно предел броска.');
        return c.flags.snipeBall ? ' ' + t('Заряженный выстрел здесь не годится — он настильный и вязнет в укрытии. Бросайте обычным.') : '';
      }
    },
    fightStep('Танк')
  ];
  ...
  var BOMBER = [
    {
      text: t('Нажмите Q: следующий снежок взорвётся при попадании.'),
    ...
    },
    {
      ...
      hint: function (c) {
        ...
        if (d && d.hp > 0 && d.hp < d.maxHp) return ' ' + t('Ящик треснул — нужен ещё один взрыв (Q, затем бросок).');
  ...
  var FREEZER = [
    ...
    {
      text: t('Бросьте его в соперника. В наледи враги двигаются заметно медленнее — вокруг замедлённого пляшут снежинки.'),
      enemy: { role: 'Танк', x: DUMMY.x, y: DUMMY.y, awake: false },
      ...
      hint: function (c) {
        return laneHint(c) || (c.inStep > 15000 ? ' ' + t('Зона наледи широкая: достаточно попасть рядом с ним.') : '');
      }
    },
    {
      text: t('Пассив «Стужа»: рядом с Фризером враги медленнее и без наледи. Пунктирное кольцо вокруг вас — граница ауры; отойдите от соперника и заведите его внутрь кольца.'),
      enemy: { role: 'Танк', x: DUMMY.x, y: DUMMY.y, awake: false },
      ...
      hint: function (c) {
        if (!c.flags.wasOut) return ' ' + t('Сначала отойдите за кольцо — так видно, как соперник замедляется на границе.');
        if (c.enemy && c.frostOn(c.enemy)) return ' ' + t('Дождитесь, пока наледь растает: сейчас работает она, а не аура.');
        return c.inStep > 15000 ? ' ' + t('Ведите его внутрь кольца и подержите там пару мгновений.') : '';
      }
    },
    fightStep('Раннер')
  ];
  ...
  var SHIELD = [
    ...
    {
      text: t('Пассив «Закалка»: щитовой пузырь гасит одно попадание целиком. Свечение вокруг бойца — пузырь готов. Встаньте на метку и дайте Снайперу попасть.'),
      enemy: { role: 'Снайпер', x: SHOOTER.x, y: SHOOTER.y, awake: true, level: 2 },
      marks: function (c) { return c.enemy ? [WALK_MARK, markAt(c.enemy)] : [WALK_MARK]; },
      ...
      // шаг ещё секунду — иначе игрок не успевает увидеть, что свечение начало проявляться, а весь
      ...
      hint: function (c) {
        if (c.flags.popAt) return ' ' + t('Пузырь лопнул — смотрите на свечение: оно проявляется по мере восстановления.');
  ...
  function create(cfg) {
    ...
    function render(c) {
      ...
      cfg.setStep(t('Шаг {n} из {total}.', { n: st.step + 1, total: steps().length }) + ' ' + step.text + hint);
